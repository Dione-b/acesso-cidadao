...
export function ProgramaCard({ programa }: ProgramaCardProps) {
  ...
  return (
    <Link
      ...
      className="group block bg-white rounded-xl p-5 card-elevate"
      id={`programa-${programa.slug}`}
    >
      {/* Cabeçalho */}
      <div className="flex items-start justify-between gap-3 mb-2">
        <h3 className="font-semibold text-neutral-900 group-hover:text-brand-700 transition-colors leading-snug">
          {programa.nome}
        </h3>
        <span
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {programa.areas.length > 3 && (
          <span className="badge badge-area">+{programa.areas.length - 3}</span>
        )}

        {programa.valor_beneficio && (
          <span className="badge badge-valor ml-auto">
            💰 {programa.valor_beneficio}
          </span>
        )}
      </div>
